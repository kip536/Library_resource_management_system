...
from instructor_login import instructorlogin
...
class instructorregister:
    def __init__(self, wind):
        ...
        self.gender = ttk.Combobox(self.window, font=('yu gothic ui semibold', 11, 'bold'), state='readonly',
                                            width=35)
        self.gender['values'] = ('Male', 'Female', 'Rather not say')
        ...
        self.school_label = Label(self.personal_frame, text="School ", bg="white", fg="#4f4e4d",
                                            font=("yu gothic ui", 13, "bold"))
        self.school_label.place(x=10, y=170)

        self.school = Entry(self.window, highlightthickness=0, relief=FLAT, bg="white", fg="#6b6a69",
                                            font=("yu gothic ui semibold", 12))
        self.school.place(x=205, y=487, width=275)  # trebuchet ms

        self.school_line = Canvas(self.window, width=280, height=1.5, bg="#bdb9b1", highlightthickness=0)
        self.school_line.place(x=205, y=510)
        ...
        # ============================emp reg no.=====================================
        # ========================================================================
        self.empreg_label = Label(self.personal_frame, text="Emp reg no. ", bg="white", fg="#4f4e4d",
                                            font=("yu gothic ui", 13, "bold"))
        self.empreg_label.place(x=370, y=170)

        self.empreg = Entry(self.window, highlightthickness=0, relief=FLAT, bg="white", fg="#6b6a69",
                                            font=("yu gothic ui semibold", 12))
        self.empreg.place(x=610, y=487, width=240)  # trebuchet ms

        self.empreg_line = Canvas(self.window, width=300, height=1.5, bg="#bdb9b1", highlightthickness=0)
        self.empreg_line.place(x=610, y=510)
    ...
    def validation(self):
        ...
        if self.username.get() == "" or self.email.get() == "" or self.password.get() == "" or self.empreg.get() == "" \
                or self.f_name.get() == "" or self.l_name.get() == "" or self.dob.get() == "" or self.department.get() == "" \
                or self.address.get() == "" or self.contact.get() == "" or self.school.get() == "" or self.register_as.get() == "":
            ...
            messagebox.showerror("Already Exists", f"{self.email.get()} Email Already Exists")
            self.email.delete(0, END)
    ...
    def click_submit(self):
        ...
        insertstudent = "INSERT INTO "+employeeTable+" VALUES('"+username+"','"+email+"','"+passwrd+"', '"+emp_reg_no+"', '"+first_name+"','"+last_name+"', '"+contact_no+"', '"+school+"', '"+department+"', '"+registered_as+"', '"+d_o_b+"','"+gender+"', '"+hom_address+"')"
        #try:
...
#createtable= '''CREATE TABLE employeetable(
    #username VARCHAR,
    #email VARCHAR,
    #passwrd VARCHAR,
    #emp_reg_no VARCHAR,
